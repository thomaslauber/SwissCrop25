...
from src.utae.convlstm import ConvLSTM, BConvLSTM
from src.utae.ltae import LTAE2d, LTAE2d_TimeChannel
...
class UTAE(nn.Module):
    def __init__(
        ...
        d_model=256,
        d_k=4,
        add_time_channel=False,
        time_encoding_method="replicate",
        ...
        num_grassland_classes=8,
        num_agriculture_classes=37,
    ):
        """
        ...
        Args:
            ...
            add_time_channel (bool): If true, a time channel is added to the input of the LTAE2d module (default False)
            time_encoding_method (str): Method for encoding time information (default "replicate")
                - replicate : add a channel with the time information to each Attention head
                - key_only : add a channel with the time information to the Key matrices
        ...
        self.temporal_encoder = LTAE2d_TimeChannel(
            ...
            d_k=d_k,
            T=T,
            add_time_channel=add_time_channel,
            time_encoding_method=time_encoding_method,
            add_doy_channel=add_doy_channel,
        )
        self.temporal_aggregator = Temporal_Aggregator(mode=agg_mode)

        if self.use_multi_head:
            self.out_conv_coarse = ConvBlock(
                nkernels=[decoder_widths[0], 32, num_coarse_classes],
                padding_mode=padding_mode
            )
            self.out_conv_grassland = ConvBlock(
                nkernels=[decoder_widths[0], 64, num_grassland_classes],
                padding_mode=padding_mode
            )
            self.out_conv_agriculture = ConvBlock(
                nkernels=[decoder_widths[0], 64, num_agriculture_classes],
                padding_mode=padding_mode
            )
        else:
            self.out_conv = ConvBlock(nkernels=[decoder_widths[0]] + out_conv, padding_mode=padding_mode)

    def forward(self, input, batch_positions=None, time_values=None, return_att=False):
        pad_mask = (
        ...
        # TEMPORAL ENCODER
        # Downsample spatial time values if needed to match feature map resolution
        if time_values is not None and isinstance(time_values, torch.Tensor) and time_values.dim() == 4:
            # Spatial time values (B, T, H_time, W_time)
            B, T, H_time, W_time = time_values.shape
            _, _, _, H_feat, W_feat = feature_maps[-1].shape

            if (H_time, W_time) != (H_feat, W_feat):
                # Downsample using adaptive average pooling
                time_values = torch.nn.functional.adaptive_avg_pool2d(
                    time_values.view(B * T, 1, H_time, W_time),
                    (H_feat, W_feat)
                ).view(B, T, H_feat, W_feat)

        out, att = self.temporal_encoder(
            feature_maps[-1], batch_positions=batch_positions, pad_mask=pad_mask, time_values=time_values
        )
        # SPATIAL DECODER
        if self.return_maps:
            maps = [out]
        ...
        if self.encoder:
            ...
            if self.use_multi_head:
                out_dict = {
                    'coarse': self.out_conv_coarse(out),
                    'grassland': self.out_conv_grassland(out),
                    'agriculture': self.out_conv_agriculture(out),
                }
                if return_att:
                    return out_dict, att
                if self.return_maps:
                    return out_dict, maps
                else:
                    return out_dict
            else:
