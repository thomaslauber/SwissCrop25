...
def confusion_matrix_analysis(mat):
    ...
    for j in range(mat.shape[0]):
        ...
        if tp + fn == 0:  # no GT pixels: phantom class, exclude from macro mean
            d['IoU'] = np.nan
        else:
            d['IoU'] = np.divide(tp, (tp + fp + fn), out=np.zeros_like(tp, dtype=float), where=(tp + fp + fn) != 0)
        d['Precision'] = np.divide(tp, (tp + fp), out=np.zeros_like(tp, dtype=float), where=(tp + fp) != 0)
        d['Recall'] = np.divide(tp, (tp + fn), out=np.zeros_like(tp, dtype=float), where=(tp + fn) != 0)
        d['F1-score'] = np.divide(2 * tp, (2 * tp + fp + fn), out=np.zeros_like(tp, dtype=float), where=(2 * tp + fp + fn) != 0)
